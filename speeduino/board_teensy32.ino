...
void initBoard()
{
    ...
    * Auxilliaries
    */
    //FlexTimer 1 is used for boost and VVT. There are 2 channels on this module
    ...
    //3.555uS resolution Min=8Hz=1000000/(3.555*36635), Max=2800Hz=1000000/(3.555*100)
    boost_pwm_max_count = 140625L / (configPage6.boostFreq); //Converts the frequency in Hz to the number of ticks (at 3.5uS) it takes to complete 1 cycle. The x2 is there because the frequency is stored at half value (in a byte) to allow freqneucies up to 511Hz
    vvt_pwm_max_count = 140625L / (configPage6.vvtFreq); //Converts the frequency in Hz to the number of ticks (at 3.5uS) it takes to complete 1 cycle
